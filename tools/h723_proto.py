...
CAP_NAMES = [
    ...
    (0x0200, "HMI"), (0x0400, "AI"), (0x0800, "MACRO"), (0x1000, "DEVBIND"),
]
...
EXPECT_CAP = (0x0001    # MULTICYCLE
              ...
              | 0x1000) # DEVBIND    (G6-4 具名设备绑定表, 2026-09-16 — transport.h 已声明, 宏=0x1DF7)
...
#           后者由 `tools/h723_dev_bind_test.py` 验收(它以 done_seq 是否跟上为准),
#           两者不要互相冒充。
#         未声明: STATE_COLD(0x0008) / HMI(0x0200) —— 见 transport.h 的两条清单。
#         (DI/HIL 与 S3 一样无专用能力位; 其存在经 0x36 自检 + SENSOR 观测证明)
# ★★★ 2026-09-16 语义分工 (新增能力位时**必须**读这段):
#   · 本常量是**等值**期望 (T1.4 = "宣称 = 实现", 多一位/少一位都算错)。
#     ⇒ 固件新增能力位 (例: `DCL_CAP_DEVBIND = 0x1000`, 实现字 0x0DF7→0x1DF7)
#       时**必须同步本常量**, 否则 T1.4 判否。这是**有意的**: 它是三处同步
#       (transport.h / 0x4A / 0x64) 的机械对账点, 松成掩码就会漏掉"宣称多一位"。
#     ★ 但它判否时的症状是"cap 不符", 语意清楚、不会把人引向接线 —— 与
#       `h723_client.link_alive` / `h723_w1` 的 T0 不同: 那两处是**认口/守门**,
#       判否的后果是"整条链路被判不活" ⇒ 已改成**必备位掩码**, 新增位不破坏它们。
#       (同一条能力字, 两种语义各司其职 —— 改之前先确认你改的是哪一个。)
# ★★ 同族事故复现记录 (2026-09-11):
