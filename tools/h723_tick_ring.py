...
def _wait_healthy(dcl, tries=HEALTH_TRIES, need=3):
    ...
    （这也解释了为什么"把同一段序列+同一载荷搬到别的脚本里重放"总是健康的:
      差别不在序列里, 而在**开局时板子处于哪种状态**。前面六个假设全部落空,
      正是因为它们都在找"序列里的差别"。）

    ## 判据与为什么它能失败
    `tick` 是 ISR 里裸自增的自由计数（`main.c:1118`），**不随 `stats_reset` 归零**
    （实测: STOP/START 后 `环写` 766126→1567 而 `tick` 继续走）。
    ⇒ `tick` 就是"本次上电活了多久"的探针。复位循环里它永远只有两位数
      （实测抓到的瞬时值: 6 / 65 / 70 / 78 —— 即上电 < 10 ms）。
    ⇒ **要求连续 `need` 次读到 ≥ `TICK_HEALTHY`** 才能开工:
      循环态**不可能**满足（它的上界只有百级）, 干净启动必然满足。
    ★ 为什么"连续 need 次"而不是一次: 一次读数可能恰好落在启动初期,
      而"连续几次都在涨且都在十万级"才是"已经稳定跑着"。

    ## 不健康时怎么办: **主动制造一次干净启动**
    关掉串口再打开 ⇒ `open_serial` 的 DTR/RTS 释放动作会给出一次复位沿
    （机制见 `h723_modbus.open_serial` 的 docstring）。实测这是**有效**的:
    同一条序列在"刚被复位过"的板上跑就从 tick=78 变成 tick=4.28e6。
    重试 `tries` 次仍不健康 ⇒ 返回 False, 由调用方**判无效**（绝不硬算）。
    """
...
def main():
    ...
    # ★★★ 2026-09-18: **自证探针**（任何其它动作之前先跑一次）。
    #   为什么必须有: 同一段序列在 `tools/h723_tick_ring.py` 里必现 `tick≈70`,
    #   而把**同一段序列 + 同一个载荷**搬到 `.tmpctl/e4_bisect2.py` 里重放却是健康的
    #   （tick 4.28e6, 静默 1.5 s 后环写 16809 = 预期值）。
    #   ⇒ 差别**不在序列里** ⇒ 只能靠工具自己报"连接那一刻 tick 是多少"来定位。
    #   判读: 刚连接就是两位数 ⇒ 板子在**打开端口**时被复位（DTR/RTS 接线, 与本工具无关）;
    #         刚连接是百万级、后来才掉 ⇒ 是本服务的某一步干的。
    #   ★ 代价: 一次 2 字读；且在 `0x38` 之前 —— 顺序本身就是被检验的变量, 不能挪。
    #   ★★ 结论（2026-09-18）: 这条探针一次就把元凶抓到了 —— **未发任何帧**时
    #     `tick` 已经是 78 ⇒ 板子在打开端口前/那一刻就在复位—恢复态里。
    #     ⇒ 补上 `_wait_healthy()`（见其 docstring: 判据、为什么能失败、怎么强制复位）。
    if not _wait_healthy(dcl):
        dcl.close()
