...
def main():
    ...
    # ── S6 阻塞路径不被污染 ──
    # ★★★ 2026-09-22 修（**假 FAIL** 族，与 `h723_i2c_sm_test` 的 J3 同源）:
    #   原判据 `b1["err"] == 0` 拿**开机累计计数器**做**绝对**断言 ⇒
    #   只要自开机以来发生过**任何一次**瞬时 I2C 错误，它就**永久**为红。
    #   症状极坏：**全量回归里必红**（前面 12 套套件都用过这条总线），
    #   而**复位后立刻单跑却是满分** —— 于是它长得像"被测对象坏了"。
    #   ★ 实测（2026-09-22）：全量回归 ⇒ `err=1` FAIL；`pyocd reset` 后单跑 ⇒ `err=0` **PASS**。
    #   ⇒ 正解与 J3 同款：**比"期间有没有新增"**，不比绝对值。
    #     判据语义本来就是"阻塞路径**不受影响**" = **没有新增错误**。
    b1 = as5600(d)
    ok_s6 = (b1["tx"] > b0["tx"] and b1["nak"] == b0["nak"] and b1["err"] == b0["err"])
    print(f"[{'PASS' if ok_s6 else 'FAIL'}] S6 阻塞路径不受影响: tx {b0['tx']}→{b1['tx']} "
          f"nak {b0['nak']}→{b1['nak']} err {b0['err']}→{b1['err']} (比'新增',不比绝对值)")
    if not ok_s6:
