...
| # | 判据 | 失败长什么样 |
|---|---|---|
| J1 | ★ **两条独立路径读数一致**：状态机读 AS5600 RAW ANGLE == 阻塞路径读的（**容差 ±1 LSB**） | 值差 **> 1 LSB** ⇒ 状态机搬错了数据（本项目纪律：*两条独立路径对上才算数*）|
...
def main():
    ...
    for i in range(a.n):
        ...
        if r["nak"] != nak0 or r["stuck"] != stuck0:
            ...
            continue
        # J1 ★ 两条独立路径读数一致
        #   ★★★ 2026-09-17 修正（判据缺陷，不是固件缺陷）: 原断言是**逐位相等**，
        #     而两条路径是**不同时刻**读同一颗 AS5600 ⇒ 在**编码器噪声底**上做等值比较
        #     ⇒ **间歇性误报**。实测: 单独跑 10/10; 在回归序列里 5/10，失败**全是差恰好 1 LSB**
        #     （`raw=1158 != 1157`）—— 那是噪声底，不是"搬错数据"。
        #     ★ 依据是本项目自己的实测: "编码器噪声底 ≈ 0.088°（±0.5 LSB，1 LSB = 0.0879°）"
        #       （H723-MOTION-QUALITY-AUDIT §1.5）。
        #     ★ 为什么加 ±1 容差**不会**放过真缺陷: 这条判据要抓的是"状态机搬错了数据"
        #       （串 lane / 字节序错 / 读错寄存器）—— 那些都会给出**远大于 1** 的差。
        #       而"误报会被自己人关掉"是本项目铁律 ⇒ 判据不能坐在噪声底上。
        v_sm = sm_raw(r["data"])
        d_lsb = (v_sm - blk["raw"]) & 0xFFF          # 12 位环绕差
        if d_lsb > 2048:
            d_lsb -= 4096
        if abs(d_lsb) > 1:
            fails.append(f"J1 #{i}: 状态机 raw={v_sm} != 阻塞路径 raw={blk['raw']}"
                         f" (差 {d_lsb} LSB；允许 ±1 = 编码器噪声底)")
            continue
        n_ok += 1
        print(f"  [{i:2d}] OK  raw={v_sm} (阻塞={blk['raw']}{'' if d_lsb == 0 else ' Δ%+dLSB' % d_lsb})"
              f"  tcnt={r['tcnt']} pcnt={r['pcnt']} "
