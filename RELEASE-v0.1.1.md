...
| 输出链路抖动（DMA 搬运 + ITCM 零等待） | **< 10 ns（实测）** | L2 抖动确定性，见 `docs/00-vision/FUTURE-ROADMAP.md` |
...
- 输出链路抖动 < 10 ns 为实测；架构上输出边沿理论下限 <0.5 ns（DMA 锁存），但尚无示波器实测记录；**循环周期级**确定性（< 10 ns 周期抖动）需外部晶振 + 硬件同步，超出当前范围
