 ...
 *
 * ★★★ 2026-09-15 更正 (原注释与实现相反, 会误导后来人):
 *   本文件原先写"**写在卡的末尾区** (LBA 3000000 起, 约 1.46GB 处), 不碰盘头的分区表/FAT"。
 *   **那是错的**。实际布局 (见 sd_log_flush_header 的 h[5]/h[6] 与 sd_log_one_batch):
 *
 *       LBA0                = 日志头
 *       LBA1 .. capacity-1  = **数据区, 环形回卷** (h[5]=1 硬编码, h[6]=整卡块数-1)
 *
 *   ⇒ **日志占满 LBA1 到卡尾, 而且会绕回来覆盖**。
 *   ★ 为什么这条更正很重要: 照旧注释去安排"程序存哪里"的人, 会把程序放到**卡开头**,
 *     那正是日志第二个回卷周期**最先覆盖**的地方。
 *     要放别的东西, **必须先给卡分区**并把日志上限关进笼子 (见 docs/PLAN-dcl-standardization.md §3)。
 *   ★ 而 sd_dump_write() 里那句 `sd_write_multi(3000000u + i, ...)` 是**一次性吞吐自检**,
 *     它写的 LBA 3000000 **落在日志区内** ⇒ 跑一次就把日志数据打穿。
 *     这是"同一张卡有两个写者"的实例, 属破坏性操作, 不应在正常路径出现。
 *
