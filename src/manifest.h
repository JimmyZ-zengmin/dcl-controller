...
static const ManifestEnt_t g_manifest[] = {
    ...
    MF_ENTRY("WDT_STAT",  OFF_WDT_STAT, 40u, MF_K_U32, MF_F_SHM),
     ...
     *      ★ 为什么阈值/开关由固件报: 工具写死就会在换档时假报警 (PR=4/RLR=99 的教训)。
     *      ★ 心跳为什么用 GPIO 不用 UART: 往主循环正在组帧的 UART 插字节会**劈开响应帧**;
     *        PE0=ISR 心跳 / PE1=主循环心跳 ⇒ 一眼区分"整机死"与"主循环死", 且不碰数据流。
