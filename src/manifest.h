...
static const ManifestEnt_t g_manifest[] = {
     ...
     *   ★ [40] **ISR 段检查点** = (段号<<28 | 拍号): 拍 ISR 每过一段就写一次 ⇒
     *     复位后读它 = **最后走完的段** ⇒ 卡点必在它之后 (调试器连接要数秒 > 窗口 ~210ms,
     *     抓不到 PC; 这是替代手段)。段: ①入口 ③时基/喂狗后 ④mb_tick后 ⑤输出段后 ⑦出口
     *   ★ [41] **上一轮**的 ISR 段检查点 (取证段在 ISR 覆盖 [40] 之前搬过来) ⇒
     *     复位后要看"上一轮卡在哪一段"必须读 **[41]** (读 [40] 只会看到当前这轮)。
