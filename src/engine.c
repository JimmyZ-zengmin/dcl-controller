...
uint32_t engine_prog_budget(const uint8_t *payload, uint16_t nr)
{
    ...
    /* ★★★ 2026-09-18（E-V）: 本函数的**摊薄口径依赖另一处的一个未声明性质**, 这里写清楚。
     *
     * ## 依赖是什么
     * `per += ceil((op+src)/mult)` 把一条 div2 路由按 `成本/64` 计入 —— 这**只有在**
     * "同档路由**均匀铺在 64 个相位**上"时才等于"最坏那一拍"。而铺相位**不在本函数里**:
     * 它在 `engine_stage_program` 尾部（`r.period = dv | ph<<2`, `ph = q2++ % PHASES_USED`）
     * —— 也就是说 **载荷里的 phase 位是被覆写的, 主机控不了相位**。
     *
     * ## 为什么必须写明（一次差点改错）
     * 我一度按"最坏相位桶"重写本函数（用的是**载荷里的** phase 位）⇒ 算出 128 条 PID div2
     * = **55296** ⇒ FLASH 档**拒**；而那条程序实际跑得很轻（铺开后最坏桶只有 **2 条 ≈ 904 cyc**）
     * ⇒ **会误拒合法程序, 最多可差 64×**。
     *   ⇒ 根因: 拿**被忽略的输入**去算一个本该由**固件自己分配**的量。
     *
     * ## 正确形态: 保留摊薄口径 + **把依赖钉成判据**
     *   判据在 `tools/exp_ev_gate_worst_phase.py`:
     *     V1 div0-only: 本函数 == "最坏相位桶"式（`mult` 就是 1）—— **逐位相等**（实测 1792/7168）
     *     V2 载荷相位**不影响**桶分布（全 phase 0 与全 phase 63 部署出**同一张桶表**）
     *     V3 铺开后**最坏桶 ≤ 本函数值**（本值即最坏拍的上界）
     *     V4 ★ 铺开后最坏桶与本函数值之差 ≤ **一条路由的成本**（`ceil` 的取整余量）
     *       现判据实测（交付档 PID div2, n=128）: 最坏桶 = 2 条 × 145 = **290 cyc**,
     *       本函数值 = **384** ⇒ 上界成立, 余量 **94 ≤ 145**。
     *       ★ 本注释**不写死跨档的数字** —— 换档/改成本表后以工具现读值为准。
     *   ★ 若哪天有人把 staging 改成"尊重载荷相位"（`PERIOD=` 的诱惑）, V2/V3 会立刻失败 ——
     *     那时**才**需要把本函数改成最坏桶式（且必须用**分配后**的相位）。 */
