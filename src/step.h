 ...
 *   ★★ 2026-09-19 再更正一次: 那个"ITCM 已满"本身就是**过期数字** —— 实测只用 14.32/64 KB
 *     （22.4%，账本 `tools/mem_report.py`）。⇒ 这条结论**两次**都建立在错前提上，
 *     靠的是"换个做法不需要中断"才走对。教训见 RETRACTIONS（假界/过期数字一族）。
 * ★ 那条**结论错了**：不需要中断 —— 让**另一个定时器去数 TIM3 的更新事件**即可。
 *   `TIM4` 的 ITR 表含 **ITR2 = TIM3** ⇒ 配"外部时钟模式 1"后 `TIM4_CNT` 就是一个**脉冲计数器**
 *   （连线全在芯片内部，不占引脚、不用接线）。
 *
 * ★ 到点怎么停：**主循环**看 `TIM4_CNT >= goal` ⇒ 停脉冲。
 ...
 *
 * ★★★ 2026-09-18（E-R/E-S 更正 —— 原注释写的是"延迟 ≤1 圈 ≈0.37 ms"）：
 *   **0.37 ms 是主循环的"名义"周期，不是上界**（拿名义值当界限 = 本项目 §5.40 那一族）。
 *   过冲的机制是"到点后要等**下一次主循环通过**才停脉冲" ⇒ 真正的界是
 *   **本次停脉冲前主循环的最大间隔** —— 而它是**可观测**的（`g_loop_gap_max`，SHM）。
 *   实测量化（`tools/exp_es_step_duration.py`，f=5000 Hz，声明 40 ms）：
 *     · 名义（主循环自由）过冲 = **0 ~ 27 ms**（5 次散布；`g_loop_gap_max` = 91.5 ms 覆盖得住）
 *     · 注入一次大块 `0x22` 读（板侧阻塞 ~100 ms）⇒ 过冲 **72 ~ 104 ms** = **282 倍于 0.37 ms**
 *   ⇒ **可算的设计规则**：要求过冲 ≤ k 步 ⇒ 必须 `f ≤ k / 主循环最大间隔`。
 *     例：间隔 100 ms、要过冲 ≤ 10 步 ⇒ `f ≤ 100 Hz`。（名义 0.37 ms 下 f=5000 Hz 的
 *     过冲上界也不是 2 步而是 `5000×0.00037 ≈ 2` 步 —— 但那个数只在"主循环不被阻塞"时成立。）
 *   ★ 另一条同时更正的**观测面事实**：「停止时刻」**不能**靠上位机量 ——
 *     轮询本身会阻塞主循环（把过冲从 0~7 ms 抬到 0~27 ms，两个条件的区间**重叠** ⇒
 *     连"方向"都不可宣称，已记 `RETRACTIONS.md`）；静默又只能在事后知道"已经停了"。
 *     ⇒ 判"声明的 N/f 时长"目前只能**组合**：`时长 = 读回步数 / 实现频率`（两者都被单独验过）。
 *     要与组合值**独立互证**，需在 `step_tick` 停脉冲那一行记一个 `g_step_stop_tick`（一行）。
