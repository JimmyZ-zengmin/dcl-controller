...
static void h_pin_pattern(const uint8_t *p, uint32_t n)
{
    ...
    if (op == 19u) {
        ...
        case 17u: {
             ...
             *   ★ 收益（实测 2026-09-17）：**突加 >16500 Hz 从静止必失速（0 rpm）**；带斜坡可达 ≥50000 Hz，
             *     且"已在转动时突跳 31 kHz"正常 ⇒ 失效纯属"从静止起转"（拉起频率 619~628 rpm）。 */
            step_set_ramp(arg);
            break;
        }
        case 19u: {
 ...
 *   （`i2c_sm_request` → 门的 acquire）, 且 800µs ≫ 拍长, 主循环一圈 **~0.37ms** 足够。
...
int main(void)
{
 ...
 as5600_init();                  /* AS5600 磁编码器: 绑定 PB10/PB11 */
    ...
    for (;;) {
         ...
         *   ⇒ 延迟放门最长拖 **10ms**（而不是 **~0.37ms**）、SHM 握手也只有 10ms 分辨率 ——
