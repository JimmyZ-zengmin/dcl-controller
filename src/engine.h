...
/* ══════════════════════════════════════════════════════════════════════════
 * ★★★ 2026-09-18 修复: **"div2 相位数"与"div2 周期拍数"是两个量, 此前被当成一个**
 *
 * ## 缺陷（实测定位, 见 `docs/exp-EH-bucket-contradiction.md`）
 * `BUCKET_DIV2_PHASES = 100` 同时被用作**两个不同的语义**:
 *   ① 桶表尺寸 / 死槽判据的右界      ⇒ 需要 **100**（表保留 100 槽是 S3 同址要求）
 *   ② **相位生成**（`q2++ % BUCKET_DIV2_PHASES`）⇒ 需要 **64**
 *     因为路由的 phase 字段只有 **6 位**（`(period>>2)` 且 period 是 u8、div 占 2 位）
 * ⇒ 生成出来的相位达到 99, 而 `(uint8_t)(ph << 2)` 在 `ph ≥ 64` 时**溢出 u8**
 * ⇒ **实测**: `n=128` 时桶表 `cnt2[64..99]` 全被填（违反自己的"恒 0"断言）,
 *   而路由表 `period` 的相位被截断到 0..63
 * ⇒ **后果: 相位 64..99 的那 36 条路由永远不会被任何一拍认领 —— 静默不执行。**
 *   实测: `n=64` 时槽 64..99 全 0（正常）; `n=100/128` 时全 1（缺陷）。
 *
 * ## 修法
 * 引入**独立**的"实际使用的相位数"常量, 并把**相位生成**与**扫描选相位**都改用它。
 * ★ 桶表**尺寸**常量保持 100 不动 —— SHM 偏移与 S3 同址是硬要求。
 *
 * ## 残留代价（**已知且未消除, 必须记住**）
 * 相位数 64 与周期 100 拍**不整除** ⇒ 一个 100 拍周期里, 相位 0..35 被选中 **2** 次、
 * 相位 36..63 被选中 **1** 次 ⇒ **每条路由的"每周期执行次数"是 1 或 2（抖动 2×）**。
 * 这比"36 条永不执行"好得多, 但**不是**理想的确定性。
 * ⇒ 要彻底干净需要**把相位字段扩到 7 位**（`div` 只占 1 位, 因为索引 3 已被
...
/* ★★★ 2026-09-22 第二次收窄：**再关掉映射槽 1（`SENSOR[1]` = deg）**。
 *   依据（实测）：运动中的产出是 **315 条/s**，而这 315 里
 *     `ch=0`(raw) 100 + `ch=1`(deg) 100 + `ch=16`(WIRE[0]) 100 + `ch=2`(HIL) 38.6 ≈ 338
 *   ⇒ **`deg` 与 `raw` 是一一对应的**（`deg = raw×360/4096`，固件 `as_write_shm` 就这么算的），
 *     传两份是**纯冗余** ⇒ 关掉它**信息零损失**，PC 侧从 raw 换算即可。
 *   ⇒ 产出 315 → **~215 条/s**，而协议侧的消费上限实测 ≈ **297 条/s**
 *     （`DELTA_READ_MAX=24` 时单次 85.1 ms、消费 297）⇒ **余量 1.38× ⇒ 不再丢**。
 *   ★ 为什么不是"加大单次"：实测 n=16/24/32 的消费分别是 281/297/283 条/s
 *     ⇒ **24 已是最优**，瓶颈是"协议往返 + 主循环占用"，不是单次大小。
 *   ★ 记法：`1`=关（0 表示上传）。默认关: 1(deg，可由 raw 导出) / 8,9,10(AI 噪声) / 25(其镜像)。 */
#define DELTA_MASK_DEF_LO   (~((1u << 1) | (1u << 8) | (1u << 9) | (1u << 10) | (1u << 25)))
...
/* ★★ 2026-09-22 调参（64 → 24）：**单次越大反而越慢** —— 实测数据说明：
 *     n=64 ⇒ 单次往返 **198.7 ms**（线路只该 90 ms；其余是"主循环里发大块"的固有代价）
 *            ⇒ 消费 64/0.1987 = **322 条/s**
 *     而运动中的产出是 **248 条/s** ⇒ 只差 6 条/s ⇒ **`DELTA_RING` 被缓慢覆盖、drop 涨**
 *     （实测 drop 0→478 / 8 s）。
 *   ★ 为什么要**切小**而不是加大：单次耗时里有一大块是"主循环被占住"的代价
 *     （组帧 memcpy + 限时 pump + SD 落盘 46.7 ms 抢时间）。
 *     切小 = 把**一段长阻塞**换成**多段短阻塞**，主循环在段间能跑别的 ⇒ **总吞吐反而升**。
 *   ★ 取 24 的依据：24×16+16 = 400 B ⇒ 线路 ~35 ms ⇒ 目标单次 ~60 ms。
 *   ★★ 这是**需要实测复核**的值（见 tools/hostsim/bridge.py 的 evt_rate/drop 两个观测量）。 */
#define DELTA_READ_MAX      24u
 ...
 *     改变另一处的语义, 而两处都叫"预算"。
 *   ★ 取值: **拍长的 80%**（100 µs 档 = 40000 cyc ⇒ **32000**, 留 8000 cyc = 20 µs 余量
 *     给热重载那一拍与协议/顺序/传感域）。范本 `core0_isr.c` 的同名量是
 *     **20000/24000 = 83%** —— 同一个口径(留余量), 数值各按各的拍长。
 *
 * ★★★ 2026-09-18(PLAN 2.3 / 耦合 #6): 原来是**手写的 32000u** —— 而本注释自己就写着
 *   "拍长的 80%"。E-U 之后拍长可配（`-DDCL_TICK_US=200`）⇒ 那个手写数在 200 µs 档上
 *   只剩 **40% 拍** ⇒ 运行期"本拍超载"判据被**静默放宽一倍**，且没有任何东西会响。
 *   ⇒ 现在**派生** + 用断言把关系钉住（见下方 _Static_assert）:
 *        · 100 µs 档: 40000 × 80 / 100 = **32000** ⇒ **逐位等于改动前的值**(交付档无回归)
 *        · 200 µs 档: 80000 × 80 / 100 = 64000 ⇒ 判据随拍长一起缩放
 *   ★ 为什么必须是**派生**而不是"改的时候记得一起改": 本项目已四次栽在"一常量两用/
 *     一处手写"上（MB_MAX_FRAME / NVIC_ISER 位移 / USART1 BRR / BUCKET_DIV2_PHASES）。
 *     "记得一起改"不是机制, 断言才是。 */
#define EXEC_BUDGET_PCT      80u      /* 拍内允许被"扫描段"吃掉的百分比 */
#define EXEC_BUDGET_CYCLES   ((CLK_TICK_CYCLES * EXEC_BUDGET_PCT) / 100u)

/* ★ 派生关系的**机器判据**（这三条是本项唯一能失败的东西, 也是它的价值）:
 *   ① 百分比必须 < 100（否则"超载"判据永不触发 = 空判据）
 *   ② 预算必须 < 拍长（必须留余量给骨架 ISR / 热重载 / 其它域）
 *   ③ ★ 100 µs 档下必须**仍然等于 32000** —— 这条把"交付档无回归"写成机器事实;
 *      谁再把它改回手写数、或改了百分比, 这里当场编译失败。
 *   ★ 换拍长（如 -DDCL_TICK_US=200）时 ①②自动成立、③**故意不成立** ⇒ 所以 ③ 只在
 *     100 µs 档下断言（用 CLK_TICK_CYCLES 判别档位, 而不是猜）。 */
_Static_assert(EXEC_BUDGET_PCT > 0u && EXEC_BUDGET_PCT < 100u,
               "EXEC_BUDGET_PCT 必须落在 (0,100) —— 100 会让超载判据永不触发(空判据)");
_Static_assert(EXEC_BUDGET_CYCLES < CLK_TICK_CYCLES,
               "运行期预算必须小于拍长(否则没有余量给骨架 ISR/热重载)");
_Static_assert(CLK_TICK_CYCLES != 40000u || EXEC_BUDGET_CYCLES == 32000u,
               "100us 档的运行期预算必须是 32000 —— 交付档不许因本项改变(逐位回归判据)");
/* ★★★ ④ 这一条才是**真正把派生关系钉死**的: 展开式必须逐位相等。
 *   为什么 ①②③ 不够: 在 **200 µs 档**上, 一个**手写的 32000u** 同时满足 ①②③
 *     （40% 拍 < 拍长; 而 ③ 此刻因 CLK_TICK_CYCLES=80000 而不生效）
 *     ⇒ 耦合 #6 会**原样复发**, 只是"没在 100 µs 档暴露"。
 *   ⇒ 有了 ④: **任何档**上手写数都当场编译失败。
 *     （③ 保留为回归哨兵: 100 µs 档下派生值必须仍是 32000 —— 防的是**百分比被改**。） */
_Static_assert(EXEC_BUDGET_CYCLES == ((CLK_TICK_CYCLES * EXEC_BUDGET_PCT) / 100u),
               "运行期预算必须是**派生量**(拍长 x 百分比) —— 手写常数会在换档时静默放宽判据");
...
/* ══════════════ 相邻 op 改变代价（2026-09-18 E-L 实测新增）══════════════
 * ## 为什么必须补
 * `engine_prog_budget()` 原来是**纯可加**（Σ 每条路由的摊薄成本）。
 * E-L 实测（n=128/div0, PID 与 DIRECT 各 64 条, **只改排列**）:
 *
 *     block  相邻op改变次数   实测 di    纯可加偏差
 *      64         1           6588.9      +8.5 TB
 *      32         3           6600.5     +20.1
 *      16         7           6622.5     +42.1
 *       8        15           6666.6     +86.2
 *       4        31           6754.6    +174.2
 *       2        63           6930.6    **+350.2**（= 700 cyc, 5.3%）
 *       1       127           6744.6    +164.2   ← 离群, 见下
 *    噪声底 = **0.0 TB**（同一程序重复三次极差 0）
 *
 * ⇒ **纯可加系统性低估、永不反向** ⇒ **它不是上界** ⇒ 做门会**放行实际超载的程序**。
 *   ★ 这与本项目前两代成本模型（标量 3.03、`min` 口径表）**是同一个方向的错**
 *     （`RETRACTIONS.md` P24/P25）。
 *
 * ## 为什么取 25 而不是拟合值
 * 每转变成本**不是常数**（8.50 → 5.56 cyc，且严格交替 block=1 反而掉到 1.29 ——
 * 那是**分支预测器能完美学习的周期模式**，与 `exp-TCM-cycles` §A2.6 当年的记录一致）。
 * ⇒ **不能用拟合值**。本常量取"**最小安全系数 × 余量**":
 *     最小安全系数（使 `纯可加 + k×转变数 ≥ 实测` 处处成立）= **17.8 cyc**
 *     取 **25 cyc**（= 1.40× 余量）⇒ 与项目在别处的余量口径一致（65% 门、80% 预算）
 *
 * ## 代价
 * ★ **对交付档零行为变化**：交付档的门本来就**不具约束力**
 *   （见下面绊线断言: 128×145 = 18560 ≤ 26000）。本项只让模型更保守。
 *   ⇒ 它的作用是**在扩容/加重量级原语时不让门变成"会放行超载"的假门**。
 * ★ 转变次数**在部署期从路由表直接数出来**，**零运行期代价**。 */
