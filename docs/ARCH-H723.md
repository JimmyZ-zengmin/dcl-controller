...
## 5.2 30 条命令（**2026-09-16 清点** —— 以 `src/main.c` 的 `proto_dispatch` 为唯一权威）

> ★ 本节原标题写"21 条"，实际列了 27 行、**代码里是 30 个命令码**。
> 差集 = 本节曾漏掉 `0x39` / `0x63` / `0x64` 三条（已补）。
> ★ 反向核对：**本节每一行都能在代码里找到**，没有"文档有、代码无"的项。
...
| 0x38 | ENGINE_STATUS | 空 → **51B**（2026-09-16 由 39B 扩展），见 §5.3 |
| **0x39** | **PIN_PATTERN** | `[op:u8]` + op 专属载荷 → 变长（6B / 46B / 64B / 70B / 96B / 102B）—— **诊断命令族，见 §5.2.1**。★ 原文漏记 |
...
| **0x63** | **MB_DIAG** | 空 → 通信域诊断区 **128B**（★ **无宏定义**，dispatch 里硬编码）。★ 原文漏记 |
| **0x64** | **SYS_MANIFEST** | `[page:u8 可选]` → `[total:u8][n:u8]` + n×20B（**诊断资源目录**，共 14 条，每页 4 条；★ **无宏定义**）。★ 原文漏记 —— 它是"**管理面入口**"：让板子自报"哪里出问题该读什么"，见 `src/manifest.h` |
...
### 5.2.1 `0x39 PIN_PATTERN` —— 诊断命令族（**诊断专用，不是正式契约面**）

载荷 = `[op:u8]` + 该 op 的专属字段；分发在 `src/main.c:2165-2394`（`h_pin_pattern`）。
未识别的 op ⇒ `nak("bad pin pattern op")`。

**★★ 交付固件里真实存在的 op 只有 8 个**（2026-09-16 逐行核实）：

| op | 请求载荷 | 应答 | 用途 |
|---|---|---|---|
| 0 | `[op]` | 空 | **关**诊断码型 |
| 1 | `[op]` | 空 | **开**诊断 + 清统计；并强制 `MDMA_CH0_CCR.EN=1` |
| 2 | `[op]` | **64B** | 引脚/统计快照（含 wr/min/max/last 与 ODR/MODER 回读、分档计数） |
| 3 | `[op][cyc:u32 LE]` | 空 | 运行期设 ISR 入口**让路延时**（交付档 = 0）—— `main.c:715` 自述"**这是诊断旋钮，不是交付配置**" |
| 4 | `[op][on:u8]` | 空 | 黑匣子开关（1 = 关 `bb_kick`），做**干涉对照**用 |
| 17 | `[op]` | **64B** | AS5600 **上电诊断**（扫 4 组候选脚对） |
| 18 | `[op]` | **40B** | AS5600 **运行态**（角度/状态/I2C 计数） |
| 19 | `[op][sub:u8][arg:u32 LE]` | **96B** | 步进控制 + **静态电平探针** + 寄存器透视（见下） |

> ★★ **`op=5/6/7/8/9/10/11/13` 不在交付固件里** —— 它们只存在于实验补丁
> `docs/exp-2026-09-14-mdma-trigger/firmware-experiment.patch`（该补丁 README 明确写着
> "只改工作区、不提交"）。**`op=12/14/15/16` 在任何源码或补丁里都查不到**（仅被部分工具/文档引用过）。
> ⇒ 叙述"0x39 支持 op=1..19"是**错的**；以本表为准。
> ★ `src/transport.h:42-47` 关于 `0x39` 的注释**仍是旧版**（只写了 op 0/1/2），需要跟平。

**`op=19` 的 `sub` 全枚举**（`main.c:2295-2336`）：

| sub | 含义 | sub | 含义 |
|---|---|---|---|
| 0 | ★ **只查询（零副作用）** | 6 | 停止 + 失能（安全态） |
| 1 | 频率 Hz（0=停）—— 报**实际**频率（由 ARR 反算） | 7 | **PA6 → 静态低 0V** |
| 2 | 方向 0/1 | 8 | **PA6 → 静态高 3.3V** |
| 3 | 使能 0/1 | 9 | PA6 → 还原 TIM3_CH1（AF2） |
| 4 | 限时 ms（0=不限） | 10 | PA6 输出类型（arg=1 ⇒ 开漏） |
| 5 | ENA 极性 | | |

**`op=19` 应答 96B 的关键偏移**：`+16` `TIM3_CCER`(CC1E) · `+40` `GPIOE_ODR` · `+48` `ena_pol` ·
`+76` `GPIOA_IDR` · **`+80` `GPIOE_IDR`** · `+84` `TIM3_CCMR1` · `+88`/`+92` **实时** `CCR1`/`ARR`
（与 `+20`/`+24` 的**缓存**值对比 —— 缓存会掩盖"别处改了寄存器"）。

> ★ **为什么 `+80` 是硬判据**：`ODR` 是"我以为写了什么"，**`IDR` 是"引脚上真实是什么"**。
> 只有 `PA6 ODR=0 而 IDR=1` 这种情况才能证明"**不是 MCU 在驱动这个脚**（高阻 + 外部上拉）"。
> ★ **`sub=0` 必须是零副作用** —— 最初 sub=0 是"停止"，于是任何只想读状态的工具
> **都在悄悄停脉冲**，症状像"限时逻辑坏了"。**读取必须是零副作用，否则它就是陷阱。**
> ★ **`sub=7/8` 的存在理由**：万用表 DC 档量 500 Hz 方波读的是**平均值**（≈1.65 V），
> "接通"与"断路"被糊在一起 ⇒ 先把 PA6 从 TIM3 摘下来输出**确定的静态电平**，再成对读两档。

> ★★ **定位声明（契约要求，尚未落到源码注释里）**：
> `docs/ASSESS-exp-standardization-2026-09-15.md` 与 `docs/PLAN-dcl-standardization.md` 要求
> **明确标注"`0x39` 是临时脚手架，不是架构的一部分"**（`op=19` 保留为兼容壳）。
> 现状：这些诊断量**未进 SHM、无 `obs_anchor()` 登记**，也**不占能力位** ⇒
> 属于**③层之外的管理面**，程序（③层）**不得依赖**它。



## 5.3 `0x38` 逐偏移（**总长 51B** —— 2026-09-16 由 39B 扩展）
...
| 39-42 | `uart_ore`（UART 硬件**溢出** ORE 次数）★ 2026-09-16 追加 |
| 43-46 | `uart_drop`（接收环写满而**丢弃**的次数）★ 2026-09-16 追加 |
| 47-50 | `frame_bad`（**坏帧**数：CRC/长度不符，含"数据没到全"）★ 2026-09-16 追加 |

**★ 破坏性变更的边界（写 release notes 时照抄）**：新增量一律**往尾部追加**，
**前 31 字节的布局与语义一个字没动** ⇒ 按 `len >= 39` 或按绝对偏移读前 39 字节的客户端**仍然兼容**；
只有"长度必须恰好 39"的客户端会失效。
**★ 为什么加这三个量**：`RX_RING_SZ=512` 而协议允许 6150 字节载荷 —— 主循环阻塞期间到达的字节
会被**丢弃并截断帧**，而这两个计数原来**只进全局量、不进任何应答面** ⇒
"帧被截断"在协议面上**完全不可见**（只能 SWD 读 DTCM，而 SWD 会停核）。
加了它们之后，"固件拒绝"与"数据没到全"才能被分开 —— 否则两者在读数上一模一样。
**★ 实现注记**：这 3 个字段的**缓冲区大小与发送长度共用一个常量** `ENG_STATUS_LEN`
（`src/main.c`）—— 因为本项曾漏改缓冲声明导致**栈越界 11 字节**，
现由 `tools/h723_ackbuf_check.py` 在**构建期**静态把关。
...
## 8.6 独立看门狗 IWDG（`src/wdt.h`，2026-09-13 落地；★ 原文对此**零提及**）

**配置**（默认值，全部可用 `-D` 覆盖做 A/B；实际生效值见 `build.sh` 打印的"生效开关"）

| 项 | 值 | 出处 |
|---|---|---|
| LSI 假设 | `WDT_LSI_HZ = 32000` | `wdt.h:62` |
| 预分频 | `WDT_PR_VALUE = 4`（`DCL_WDT_PR`） | `wdt.h:74` |
| 计数步长 | `WDT_STEP_MS = 4×2^PR×1000/LSI = **2 ms**`（整数截断） | `wdt.h:76` |
| 超时 | `WDT_TIMEOUT_MS = **200 ms**` ⇒ `WDT_RLR_VALUE = 200/2−1 = **99**` | `wdt.h:66/77` |
| 等更新落 | `WDT_SYNC_BUDGET_CYC = 40000000`（**100 ms** @400MHz，周期级死线） | `wdt.h:57` |

**喂狗点（只有两处，且都不在主循环）**
1. **主喂狗点 = 100 µs 拍中断**：`main.c:995`（`TIM2_IRQHandler` 内），**每拍一次**；
   喂前先过闸门 `g_wdt_kr_busy`（`main.c:983-998`）。
2. **有界喂狗 = flash 等待循环**：`flash.c:96`（`fl_wait_qw` 内，**在超时判据之后**）
   ⇒ 只在预算内喂，超预算即停止喂狗。

★ **主循环不喂狗**（这是契约，不是巧合）：主循环一圈实测 **0.37 ms**（2026-09-16 **直接实测**；原 1.28~1.54 ms 是**反推值**，已作废），
若让它喂狗，则"主循环卡住"这件事**永远不会被看门狗发现**。把喂狗钉在拍 ISR 上，
才让"ISR 不返回 / 拍停摆"变成**可被硬件复位**的故障。

**超时后行为**：硬件复位，`RCC_RSR.IWDG1RSTF`（合法复位签名 `0x04420000`）。
另一条**独立**路径：主循环停滞自愈（软复位 `SYSRESETREQ`，`DCL_LOOP_RESET=1`，
阈值 `LOOP_STALL_TICKS=12000` 拍 = 1.2 s）。

**可观测量**：`WDT_STAT` @ **`OFF_WDT_STAT=0x70B8`**（`engine.h:557`，**48 字 = 192 B**）
⇒ `0x22 READ_BURST`（addr = `g_shm+0x70B8`，count=48）；`0x64 SYS_MANIFEST` 有目录条目。
字段含：`g_wdt_armed` / `wdt_ms` / **喂狗计数**（ISR 里自增）/ `IWDG_PR/RLR/SR` 实测回读 /
`sync_ok` / `timebase_dead` 等。

**编译开关**：`DCL_WDT=1`（交付）· `DCL_WDT_FEED_GATE=1` · `DCL_WDT_START_FIRST=1` ·
`DCL_WDT_PERSIST_WINDOW=8000`（落盘窗口 ms）。
★ **不占能力位**（`DCL_CAP_*` 里没有 WDT 位）⇒ **`0x01 GET_VERSION` 判不出看门狗在不在**。

**★★ 运行期纪律（最容易吃亏的一条）**：看门狗武装后，**任何 ≥200 ms 的调试器 HALT 都会复位板子**。
⇒ 检查一律走**协议**；确需 SWD 时必须先把 `DCL_WDT` 关掉或把超时放大。
判据/坑位全集见 `docs/VALIDATE-watchdog.md`（含两个"会撒谎的判据"与负向对照）。

---

## 8.7 RTC（`src/rtc.c`，2026-09-12 落地；★ 原文对此**零提及**）

**结论：已完整初始化且确实在被使用**（不是"只有骨架"）。

| 项 | 值 | 出处 |
|---|---|---|
| 时钟源 | **LSE 32768 Hz**（等 `LSERDY`，超时上限 ~2 s） | `rtc.c:56` |
| 预分频 | `PRER = (127<<16) \| 255` ⇒ `PREDIV_A=127`(128 分频) / `PREDIV_S=255`(256 分频) | `rtc.c:105` |
| 日历频率 | `ck_spre = 32768/128/256 = **1 Hz**` ✓ | `rtc.c:104` |
| **亚秒计数** | `ck_apre = 32768/128 = **256 Hz**` ⇒ **分辨率 ≈ 3.9 ms** | 同上 |
| 初始化 | `rtc_init(g_shm)`，`main.c:3736`（无条件编入，**无编译开关**） | — |
| 刷新 | `rtc_latch()` 在**拍 ISR** 内，`main.c:1418`（内部 /64 降频 ⇒ ~156 Hz，避免每拍写） | — |
| 谁在用 | **黑匣子 SD 日志的挂钟时间戳**（`BB_MAP_SEG_TIME` ← `OFF_RTC_TR/DR`） | `blackbox.c` |

**★★ 频率声明的更正（本轮实测）**：`rtc.h` / `rtc.c` 文件头、`engine.h:527`、`manifest.h:89`
原来都写 **1024 Hz / ~1 ms**，**与 `rtc.c:105` 实际写入的 `PRER` 不符**——
权威值是上面的 **256 Hz / ≈3.9 ms**（已四处更正为 256 Hz）。
★ 凡按"~1 ms"做时间差的判据都要复核。若要真 1 ms：`PREDIV_A` 改 31、`PREDIV_S` 改 1023
（日历仍是 1 Hz）—— 但那是**行为改动**，需先定"SSR 要什么分辨率"，故本轮**只改文字、不动代码**。

**可观测量**
- `OFF_RTC_DIAG = 0x4A00`（`u32[4]`）：初始化取证（BDCR / ISR / 启动时 TR / 状态判定）。
- `OFF_RTC_SSR/TR/DR = 0x6E10 / 0x6E14 / 0x6E18`。
- **读当前时间**：`0x22 READ_BURST`（addr = `g_shm+0x6E10`，count=3）。
  ★ **注意**：`0x64` 目录里**没有** `RTC_SSR/TR/DR` 条目 ⇒ 当前时间只能**按硬编码偏移**读，
  与 `manifest.h` 自己宣称的"消灭 PC 端硬编码地址"目标**不一致**（记为待办）。
- **不占能力位**（无 RTC 位）⇒ `GET_VERSION` 判不出 RTC 在不在。

**★ 未接线项（别写成"已工作"）**：`event_log(code)`（`rtc.c:175`）**全仓零调用点**
⇒ 所谓"关键事件时间戳"目前**只有 API、没有被写入**；事件环 `EVT_BUF` 实际为空。
现状只到"**亚秒/日历镜像被刷新、黑匣子取到了挂钟**"。
