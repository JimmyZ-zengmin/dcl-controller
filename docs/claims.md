...
> | **F** | *（自动）* | **门面/状态一致性**：① 表格行**不得自相矛盾**（现状说"已完成"、状态列写"未完成"）② 提到"实现字/能力字"的行，**末值必须等于** `transport.h` 的 `DCL_CAP_H723_IMPL`；③ **门面里的本地链接必须指向真实文件**。★ 补它的原因：`claims` 只管"契据⇄代码"，**管不到 发布说明/GAP 表/README** —— 同一天真的出现过 6 处过期状态，而**门面写错比没写更坏** |
