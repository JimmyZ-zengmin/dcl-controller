...
DTCM 里的 `g_shm`（**32KB；地址由链接器分配，不要写死**）。**所有跨域数据交换都经过它, 没有例外**
...
| **0x4AA0–0x4DE0** | MB 区 (Modbus 帧/保持寄存器) | 832B | 协议 / ISR | 协议 |
...
| `op` | **算什么**: 19 原语之一 (PID/LPF/**TIMER**/CMP/MUX/…) ★ 是 `OP_TIMER`(`engine.h:770`); `TON/TOF/TP` 是它的**模式**(`value_b`), **不是独立原语** —— 本行原来写的 `TON` 是错的 |
...
|---|---|---|---|---|
| 快 | FAST | **每拍** (100µs) | `DT_FAST` = 0.0001f | 快速逻辑/联锁 |
| 中 | MID | **每 10 拍** (1 ms) | `DT_MID` = 0.001f | 一般闭环 |
| 慢 | SLOW | **每 100 拍** (10 ms) | `DT_SLOW` = 0.01f | 慢回路/滤波 |

> ★★ **2026-09-16 更正**: 上表原写"中 = 每 2 拍(200µs) / 慢 = 每 4 拍(400µs)"，**是错的**。
> 权威值是 `engine.h:190-191` 的 `BUCKET_DIV1_PHASES=10` / `BUCKET_DIV2_PHASES=100`
> 与 `engine.h:851-853` 的 `DT_FAST/DT_MID/DT_SLOW` —— 周期与 dt **必须同源**，
> 原来那组数字与同一份文档里给出的 `dt` 值**自相矛盾**（2 拍不可能对应 1 ms）。
> 这个错误会把"分档调度到底在什么节拍上跑"整体判偏，属会误导实现的那一类。
...
3. **`phase` 只有 6bit(0..63)** —— ★ 与慢档的 **100 拍**周期**不匹配**：桶表有 100 个
   div2 槽，而 `phase` 只能表示 0..63 ⇒ **槽 64..99 永远空**（36 个死槽）。
   本行原写"慢档 4 拍下够用"，前提本身就是错的（见上表更正）。
   死槽由 `engine_bucket_dead_slots()` 恒 0 断言兜住；记为**已知 H9**
   （`engine.h:172-203` / `CORE-ENGINE.md` §7③）。要用满必须扩 `phase` 字段宽度。
