> ## ⚠️ 归档说明（2026-09-16）
>
> 这是 **v2.1.0 交付之前**的长篇 README（含 2026-09-10 → 09-15 的逐日状态流水、旧命令、
> 旧端口号 `COM11/COM14`、以及"12 套 195 PASS"这类**已被分项取代**的总数）。
> ⇒ **本文件保留作历史，不要当作现行说明**。现行入口见仓库根 `README.md` 与
> `docs/GETTING-STARTED.md` / `docs/SUPPORTED-SCOPE.md`。
> ★ 其中**仍然成立、不要动**的部分：铁律 0（非侵入式交互）· 硬件与仪器表 · CH340 接线表 ·
> 错误码闪灯（`src/clock.h` 的 `CLK_ERR_*`）。

---

# DCL — 确定性控制运行时（STM32H723 / Cortex-M7）

> ## 📌 最新版本入口（2026-09-16）→ **[`docs/RELEASE-v2.1.0.md`](docs/RELEASE-v2.1.0.md)**
>
> ★ **先看这一页：** **[`docs/SUPPORTED-SCOPE.md`](docs/SUPPORTED-SCOPE.md) —— 可信范围与使用入口**
> （能依赖什么、**明确不要依赖什么**、每条能力怎么自己验、遇到问题怎么救、状态从哪看）
>
> 那一页有：**30 秒上手** · **破坏性变更**（上位机必须处理）· **每个数字对应的复跑命令** ·
> **已知问题清单**（含"为什么可以留着"）。本版做成的三件事：
>
> | | 一句话 | 怎么验证 |
> |---|---|---|
> | **接新器件** | = **上传一段配置**（不用改固件、不用重烧）| `python tools/h723_dev_bind_test.py --port COM21` → **52/0/1** |
> | **配置随程序包持久化** | 开机自动恢复，老包行为**逐字节不变** | `python tools/h723_devbind_persist_test.py --port COM21` → **32/0** |
> | **应答带归属** | 帧里回显 `CMD+SEQ`，不再吃错帧 | `python tools/h723_frame_attrib_test.py --port COM21` → **18/0** |
>
> 验证总账：**6 道构建闸门 + 5 套上机验收 + 1 条泄漏判据（共 126 条）全绿**，且**每条判据都能失败**
> （变异体 + 反空判据）。★ **唯一必须注意**：认口用**必备位掩码**（`(cap & 0x0DF7) == 0x0DF7`），
> **不要写等值** —— 能力字随版本增长（本版 `0x7DF7`），等值判据会让你以为"板子没响应"。
>
> ---
>
> 下面这段"已修正"横幅是**历史文档的就地加注**，按它自己的规矩**保留**（与上面这版不冲突：
> 它标的是 2026-09-15 之前的旧宣称）。

> ## ⚠️ 已修正（2026-09-15 加）
>
> 本文有**三处宣称**已被后续实验/实现推翻或需要口径限定。正文不改，**就地加注**（见下方对应位置）。
> **最新实测总账见 `docs/audit/H723-MOTION-QUALITY-AUDIT.md`（HEAD 版）+ `docs/ARCH-TIMELINE-CPU-MDMA.md` 顶部修正段。**
>
> | 本文的说法 | 现状 | 依据 |
> |---|---|---|
> | "★ 验收套件（12 套，195 PASS / 0 FAIL）" | 该总数**已无处可复跑**；后续被 `h723_audit_full` 12 PASS、`h723_demo_e2e` 20 PASS、S3 回归 22 PASS / 7 N/A 等**分项**取代 | 本文"阶段 6/7/8"分项 |
> | "persist 落盘路径已打通并实测可用" | 与同一份 README 后文"内部 flash 掉电保持在本平台不可靠 / `0x43` 落盘 ⇒ NAK"**自相矛盾**；**以后者为准** | 本文"阶段 3"第 3 条 / `docs/audit/H723-PERSIST-WDT-DEFECT.md` |
> | "拍周期抖动 min = max = 40000 cyc（< 2.5 ns）" | 该值**是 ISR 拍周期、本身成立**，但极易被读成"输出引脚零抖动"；而**"输出时刻由硬件锚定"已被推翻** ⇒ 需加口径限定 | `docs/exp-2026-09-14-mdma-trigger/README.md` |
>
> **仍成立、不要动**：★ 铁律 0（`DWT_CYCCNT` 会被 pyocd 静默停掉）· persist 降级说明 · `itcm.h` 闸门 6 bug 教训。

**一个用「静态路由表」替代「程序」的裸机控制运行时。**

没有 RTOS、没有调度器、没有 `while(1)` 大循环。全系统只有一个 **100 µs 的硬件定时硬拍**；
拍内按拓扑序扫描一张**编译期生成的静态路由表**，逐条完成「读源 → 算原语 → 写目标」，
结果直接落到 GPIO / PWM / 通信。因为表中没有动态分支、热代码全部住在零等待 ITCM，
**每拍要花多少周期是编译期就能算出来的量**，而不是"通常够快"。

运行时一次烧录，控制逻辑（一张表）可以反复下发覆盖 —— **热更新 ≤1 拍。**
...
| 路线 | 怎么拿到确定性 | 代价 |
|---|---|---|
| **PLC / 软 PLC** | 把周期拉到 ms 级，用一个很长的周期"藏住"抖动 | 响应慢；想做 µs 级闭环就必须换路线 |
| **运动控制卡** | 上专用硬件（FPGA / DSP）做 µs 级闭环 | 闭源、专用、改不动，且通常只做闭环这一件事 |

本项目走**第三条路：在通用 MCU 上，用「架构」而不是「专用硬件」消掉不确定性。**
它的主张是：不确定性有明确的来源（动态调度、cache miss、分支预测、flash 取指），
**把这些源逐个移出热路径，通用 MCU 也能拿到可预算的执行时间。**

---

## 靠三件事做到

1. **硬件定时锚点** —— 100 µs 由定时器强制，不在软件里。软件只回答"这一拍要算什么"。
2. **表驱动、无动态分支** —— 编译器用 Kahn 拓扑排序保证生产者先于消费者；
   执行时间在编译期可预算（`128 条 × 单条成本` 就是个上界）。
3. **零等待内存** —— ISR 与热代码在 **ITCM**，路由表在 **DTCM**，
   把 cache miss 这个最难预测的不确定性**移出**了热路径。

---

## 实测到什么程度（每条都能用仓库里的脚本复跑）

| 指标 | 实测值 | 口径 |
|---|---|---|
| 拍周期 | **40000 cyc = 100.0000 µs** | 外部逻辑分析仪复核（众数 96% 命中） |
| 拍周期抖动 | **min = max = 40000 cyc**（< 2.5 ns） | DWT，经 LA 外部标定；已低于 LA 分辨率 · **⚠️ 见头部横幅：这是 ISR 拍周期口径，不代表输出引脚零抖动（"输出时刻由硬件锚定"已被推翻）** |
| 单条路由成本 | **56.0 cyc = 140 ns** | 同镜像 FLASH vs ITCM A/B |
| 128 条全表扫 | FLASH **26229 cyc（占拍 65.8%）** vs ITCM **7225 cyc（18.3%）** | 同上 |
| 分档调度 | 每拍只跑 **47~49 条**（比全表扫便宜 **2.59×**） | 三档程序 A/B |
| 热更新 | **≤1 拍**原子切换 | 双副本 + STAGING |
| 容量 | 表上限 128 条 / 拍预算 471 条 | 仿射拟合残差 5.3 cyc |

★ **口径声明（重要）**：时序类结论以**外部逻辑分析仪**为准（LA 时基精度约 0.1%，
所以读作"0.1% 量级一致"）；**凡是没实测的数值一律不进这张表**。
本项目对"把设计目标写成实测结果"有明确纪律 —— 每条判据都要能**失败**，
仪器本身也要先证明"它能报出错"（详见下方"证据纪律"一节）。

---

## 它**不是**什么（请先读这段）

- **不是工业 PLC 替代品**：无安全认证路径（单核无锁步，器件最多 SIL-2 且需自行论证）、
  无电气隔离、无 EMC 验证、无器件寿命承诺。
- **不是运动控制卡**：ADC 目前用芯片内置、10ms 轮询级（不是 50kHz 采集）；
  闭环只在 bench 验证过，没有挂真实被控对象跑过 72h。
- **不追求"指标最先进"**：它证明的是**路线可行 + 证据可复跑**，不是"比谁快"。
...
>
> 如果你在找"能直接上现场的东西"—— 这不是。
> 如果你在找"**一个把确定性做成可测量、且每条结论都能自己复跑的裸机运行时**"——
> 这里有全套代码、A/B 对照脚本，和一份**包含失败记录**在内的完整档案。

---

## 证据纪律 —— 可能比指标更值得看的部分

指标会过时，**产生指标的方法**不会。这个项目在这件事上交过学费，所以立了四条规矩：

**① 判据必须能失败。** 一个"任何输入都通过"的判据等于没有判据。
- 热更新验收：判据不是"回了 ACK"，而是**双副本校验和与 Python 独立实现的预测逐位吻合**；
- 分档调度验收：本拍扫过的条数**必须落在桶表允许的集合 `{47,48,49}`** ——
  若退回全表扫，条数变 128，判据立刻报错。

**② 仪器自己也要先被证明"它能报出错"。** 这一条是血换来的：
ISR 落位闸门（检查"哪些函数会被拍中断调到、却还留在 flash"）曾**输出"无违规"**，
而真相是它自己有 **6 个 bug**。最坏的一个是**读字面量没做字节序反转**，
于是"目标在不在 flash"的判据**恒为假**，却照样打印"检查了 220 个字面量"。
修掉后，同一个 ELF 立刻从 **0 违规**报出 **8 处违规**。
⇒ **仪器报"通过"，不等于它检查过。**

**③ 每个特性都要有"改前行为"的对照档。** 只跑交付档看到 PASS 不构成证据 ——
无法排除"测量方法本身量不出差别"。
- 中断向量表搬进 ITCM 的效果，用 `-DDCL_VTOR_ITCM=0/1` 两档打出来：
  **留在 flash 丢 8153 拍 / 进 ITCM 丢 0 拍**。对照组能量到缺口，交付版的"零丢失"才算数；
- 看门狗、停机安全态、落盘窗口，每个都配了能打出 FAIL 的对照构建。

**④ 时序结论必须有外部仪器，且未实测的量不进指标表。**
SWD 在 CPU 跑不住时会失步并给出**完全错误**的结论（本线亲历：SWD 报"300~340MHz 断崖"，
LA 实测 450MHz 正常）。

**失败记录也在仓库里**：`docs/audit/` 含 4 轮外部审计与多份缺陷报告，
其中包含**作废结论**（标注"某结论已作废，原因是工具缺陷"）与未闭合项清单。
它们是这个仓库最诚实的部分 —— 记录踩过的坑本身有价值。

---

> **版本沿革**
>
> 本仓库是 DCL 引擎在 H723 平台上的**整合重写版本**（前身从 ESP32-S3 迁移而来，
> 与历史探索项目 `../h723-core0` **无代码继承关系**，只借鉴其"硬件事实"级发现）。
> 早期版本是开发过程记录，已原样保留在
> [`dcl-controller`](https://github.com/JimmyZ-zengmin/dcl-controller) 的 **`legacy/dev`** 分支，
> **不建议引用其中指标**。
>
> 本版本的差别在**证据纪律**：每条结论都要能被命令复跑，且**仪器本身也要有"能失败"的判据**。
> 起因是旧版曾把**设计目标写成了"实测结果"**（最典型的是"输出抖动 < 10 ns" ——
> 全库无示波器记录、相关文档里它原本带问号），详见 `legacy/dev` 分支 README 顶部的交代。
...
- ✅ **阶段 1 空拍骨架**（详见 `docs/STAGE1-REPORT.md`）：
  ...
  **拍周期抖动 = 0 个 CPU 周期**（min = max = 40000 cyc，即 <2.5ns，比 LA 的 62.5ns 更细）**（⚠️ 见头部横幅：此项是 ISR 拍周期口径，不代表输出引脚零抖动）**；
...
#   ⚠️ 见头部横幅: 该总数已无处可复跑, 已被 h723_audit_full / h723_demo_e2e / S3 回归 等分项取代。
...
3. 🟡 **persist —— 落盘路径已打通并实测可用, 但默认仍关闭**（2026-09-13 夜, 上板实测）
   **（⚠️ 见头部横幅：此说法与后文"内部 flash 掉电保持在本平台不可靠 / `0x43` 落盘 ⇒ NAK"自相矛盾，以后者为准）**
...
- ★★ **内部 flash 掉电保持（平台级阻塞, 非软件缺陷）**：落盘路径已修好并实测可用
  **（⚠️ 见头部横幅：交付档 `DCL_PERSIST_SAVE=0`，`0x43` 落盘明确 NAK —— 以交付档为准）**
