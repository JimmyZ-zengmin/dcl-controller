...
1. **一切数据都在 SHM 里**（DTCM 里 32KB 的一块，**地址由链接器分配 —— 不要写死字面量**；运行时从 `0x38` 应答的 `shm` 字段取）。组件之间不传指针、不加锁 —— 通信就是"写我自己的槽"。
...
- **div0**: 每拍全跑；
- **div1**: 每 **10 拍（1 ms）**，**10 个相位桶**（`BUCKET_DIV1_PHASES=10`；`DT_MID=0.001f`）；
- **div2**: 每 **100 拍（10 ms）**，**100 个相位桶**，但 `phase` 字段只有 6bit（上限 63）
  ⇒ **槽 64..99 恒空**（已知 H9）。
  ★★ **2026-09-16 更正**：原写"div1 = 每 2 拍 / div2 = 每 4 拍"，**两处都错**。
  权威值 = `engine.h:190-191` 的桶数与 `engine.h:851-853` 的 `DT_FAST/DT_MID/DT_SLOW`
  —— **周期与 `dt` 必须同源**，原来那组数字与同页给出的 `dt` **自相矛盾**。
  （★ `OP_COST_DIV2=64` 是**刻意的**，不是笔误：按相位容量封顶，`engine.h:691` 有注。）
