...
| **A** | **"走 N 个脉冲自停"** | ✅ **能，而且可以完全不动 ITCM** | `TIM3` 的 **UEV 触发 DMA**，`NDTR = N`，**目的 = `TIM3_CCER.bit0` 的位带别名**（`0x42008400`）⇒ 搬满 N 次后**硬件自己把 `CC1E` 清掉**。★ 到位中断是"**每 N 步一次**"而非"每步一次" ⇒ **不吃 ITCM**；用 MDMA 链表两段式可**彻底免中断**（顺带把现在**空转在自循环的 MDMA 用起来**）|
