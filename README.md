...
1. [零抖动：计算与输出解耦](docs/核心思想-零抖动方案.md) — 输出边沿为什么稳（实测 < 10 ns）
...
- **数据的时间语义**：所有输入由 DMA 在周期开始前锁定为**上一周期末尾的完整快照**——周期内所有路由读同一份数据（零撕裂）；周期 N 的输出基于周期 N-1 末输入（一拍延迟，与 PLC 扫描 / 运动控制器语义一致）。见 `docs/00-vision/CORE-THEORY.md`（火车模型）
