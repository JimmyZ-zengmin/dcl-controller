...
> 在通用 MCU 上研究"接近硬件级确定性"的控制引擎：100 μs 硬件定时周期、**物理输出抖动 < 0.5 ns**（DMA 硬件锁存，与 CPU 计算解耦）、用一张"路由表"而非"程序"驱动 GPIO/PWM。
>
...
| **物理输出抖动** | **< 0.5 ns**（DMA 硬件锁存） | CPU 写 SHADOW → TIM1_CC4 → DMA2 → GPIOE_ODR，输出时刻由硬件决定，与 CPU 入口无关。**这是真正的输出质量指标** |
...
**诚实对标**（**物理输出抖动**，详见 `docs/核心思想-零抖动方案.md`）：

| 设备 | 周期 | 输出抖动 | 说明 |
|---|---|---|---|
| **DCL（H723）** | 100 μs | **< 0.5 ns** | DMA 硬件锁存，输出与 CPU 解耦 |
| 西门子 S7-1200 | 1 ms | ~25 ns（循环抖动） | DCL 快 ~200–2000×，输出抖动更小 |
| Beckhoff BX5100 | 50 μs | ~5 ns（循环抖动） | 输出抖动同级或更优 |
| B&R X20 | 100 μs | ~10 ns（循环抖动） | 输出抖动更优 |
| FPGA (Zynq) | 1 μs | ~0.5 ns | DCL 输出抖动已到 FPGA 量级 |

> ⚠️ **关于 37 ns 的澄清**：README 早期版本把"~37 ns 周期采样抖动"（2026-07-14 旧报告 `docs/JITTER-MEASUREMENT.md`，测的是 ISR **入口**时刻，且 TIM1@136MHz 与 DWT@240MHz 跨时钟域有量化台阶）当成了输出指标，这是**测量方法误导**——它既非 GPIO 输出抖动，也与最终输出质量无关。本项目**真正的输出指标是 < 0.5 ns**（DMA 硬件锁存）。
...
**诚实边界**：早期报告里的「~37 ns 周期采样抖动」测的是 *ISR 入口* 时刻的波动（且跨时钟域，见 `docs/JITTER-MEASUREMENT.md`），**不代表 GPIO 输出质量**；**物理引脚输出抖动实际 < 0.5 ns**（仅受晶振与锁存延迟影响）。周期采样的抖动 ≠ 输出的抖动——输出时刻由硬件锁存，与 CPU 入口无关。完整推导与代码位置见 👉 **[`docs/核心思想-零抖动方案.md`](docs/核心思想-零抖动方案.md)**。

**核心思想三部曲**（写给第一次点进来的访客，从"为什么牛"讲起）：

1. [零抖动：计算与输出解耦](docs/核心思想-零抖动方案.md) — 输出边沿为什么稳（< 0.5 ns）
...
- **确定性三层次**（详见 `docs/FUTURE-ROADMAP.md`）：
  ...
  - L2 抖动确定性：DMA 搬运 + ITCM 零等待 ✅ 已实测（物理输出 <0.5 ns）
