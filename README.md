...
## ★★★ 一、本版最硬的实证（**2026-09-17 真机跑出来的**，每条都能自己复跑）
...
> ★ **诚实标注**：轨迹规划的**形状验收尚未完成** —— PC 侧差分测速在本装置上被证明不可用
> （编码器回填率与 PC 轮询率发生**采样拍频**，实测 `Δraw` 中位数与理论差 60 倍），
> 必须改用**片内黑匣子**重做。见 [`docs/PLAN-closedloop-stepper-v2.md`](docs/PLAN-closedloop-stepper-v2.md) §阶段3。
...
| | 常见做法 | 本运行时 |
|---|---|---|
| 确定性怎么来 | PLC/软 PLC：把周期拉到 **ms 级**，"藏住"抖动 | **不藏**：拍固定 **100 µs**，**每拍成本编译期可算**，热代码在零等待 ITCM |
