...
│   ├── h723-core0-1us/    # 研究：1 μs 极限验证版（JIT 编译路由表到 ITCM，已跑通）— 证明架构边界，非生产配置（生产用 100 μs）
...
4. **1 μs 极限验证版已跑通（非生产配置）**：`h723-core0-1us` 用 JIT 编译块 + ITCM 一致性修复（`DSB+ICIALLU`）实测跑通 1 μs（136 MHz / 136 cycles）；它证明架构边界，日常部署仍用 100 μs（~8 ns 抖动、可长期稳定）。
