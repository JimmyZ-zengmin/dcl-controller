> # ⚠️ 这是**开发过程版本**（historical / development snapshot）
>
> 本分支是**早期开发过程**的记录，**文件组织与结论口径都比较混杂**，请勿直接引用其中指标。
>
> **具体说明（诚实交代）**：其中一部分"实测"结论，是当时用**有缺陷的验证工具**得出的，
> 与后来的实测不一致。最典型的一条是 **"输出链路抖动 < 10 ns（实测）"**：
> - 本仓库内**没有任何示波器实测记录**（全库检索 `示波器/oscilloscope` 只命中计划文档）；
> - 相关文档（`DCL-HPC/`、`docs/00-vision/`）里，它原本是写成 **`抖动 < 10ns?`（带问号的待验证项）**，
>   在 README/CHANGELOG 里被当成了"实测结果"；
> - 量级上也不成立：如果用逻辑分析仪测量，16 MHz 采样率的时间分辨率是 **62.5 ns**，
>   **物理上不可能测出 < 10 ns 的抖动**。
>
> ⇒ 那一类结论**应视为作废**（同类问题还有"周期采样抖动 37 ns 被当成输出指标"，README 内已自行澄清过一次）。
>
> ---
>
> **✅ 整合后的版本请见 [`main` 分支](../../tree/main)。**
> 新版本在同一批问题上采用了可复核的口径（外部逻辑分析仪复核 + A/B 对照 +
> "**判据必须能失败**"），并显式标注了测量不确定度。
>
> 保留本分支是为了**可追溯开发过程**，不做任何美化 —— 记录踩过的坑本身有价值。
