...
| **B** | **多轴** | ⚠️ **能，但要扩架构**（`step.c` 单例 → 实例化；插补放 ③ 层）| ★ **三个真阻塞**：① 第二个编码器（AS5600 **地址固定 0x36** ⇒ 需 I2C mux/第二总线）② 第二个驱动器/电机（**台架没有**）③ **ITCM 64/64 KB 已满**。<br>启动清单见 [`docs/PLAN-multi-axis-v1.md`](docs/PLAN-multi-axis-v1.md) |
